...
class GetEventsOperation: GroupOperation {
    // MARK: Properties
    
    let downloadOperation: DownloadEventsOperation
    let parseOperation: ParseEventsOperation
    ...
    */
    init(context: NSManagedObjectContext, completionHandler: Void -> Void) {
        ...
        */
        downloadOperation = DownloadEventsOperation(cacheFile: cacheFile)
        parseOperation = ParseEventsOperation(cacheFile: cacheFile, context: context)
