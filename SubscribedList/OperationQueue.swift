...
class OperationQueue: NSOperationQueue {
    ...
    override func addOperation(operation: NSOperation) {
        if let op = operation as? Operation {
            ...
            let delegate = BlockObserver(startHandler: nil, produceHandler: { [weak self] in
                    self?.addOperation($1)
            ...
            */
            ...
            if !concurrencyCategories.isEmpty {
                ...
                let mutuallyExclusiveQueue = MutuallyExclusiveQueue.sharedMutuallyExclusiveQueue

                mutuallyExclusiveQueue.addOperation(op, categories: concurrencyCategories)
                
                op.addObserver(BlockObserver { operation, _ in
                    mutuallyExclusiveQueue.removeOperation(operation, categories: concurrencyCategories)
                })
            }
